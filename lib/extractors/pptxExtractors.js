...
export class PPTXThumbnailExtractor {
  ...
  async extractThumbnail(filePath, options = {}, tempDir) {
    ...
    if (slideContent) {
      ...
      if (imageData && imageData.data) {
        return await this.saveFileAndResize(imageData, options, tempDir);
      }
    }
    ...
    for (const thumbnailPath of thumbnailPaths) {
      if (zip.files[thumbnailPath]) {
        const thumbnailData = await zip.files[thumbnailPath].async('nodebuffer');
  ...
  async saveFileAndResize(imageData, options = {}, tempDir) {
    const baseName = 'pptx_thumbnail';
    const tempFilePath = path.join(tempDir || '', `${baseName}.png`);
